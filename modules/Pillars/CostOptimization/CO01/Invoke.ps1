...
Register-WafCheck -Pillar 'Cost Optimization' -Id 'CO:01' -Name 'Culture of financial responsibility' -Description 'Budgets and alerts present' -InvokeScript {
  param([string]$SubscriptionId)

  $budgets = Get-AzConsumptionBudget -ErrorAction SilentlyContinue
  $status = ($budgets.Count -gt 0) ? 'Warn' : 'Manual'
  New-WafResult -Pillar 'Cost Optimization' -Id 'CO:01' -Name 'Culture of financial responsibility' -Description 'Budgets present (proxy)' `
