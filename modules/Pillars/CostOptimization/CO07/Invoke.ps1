...
Register-WafCheck -Pillar 'Cost Optimization' -Id 'CO:07' -Name 'Optimize component costs' -Description 'Advisor rightsizing + unused resources' -InvokeScript {
  param([string]$SubscriptionId)

  $adv = Get-Advisor -SubscriptionId $SubscriptionId -Category @('Cost')
  $status = ($adv.Count -gt 0) ? 'Warn' : 'Pass'
  New-WafResult -Pillar 'Cost Optimization' -Id 'CO:07' -Name 'Optimize component costs' -Description 'Advisor cost recommendations' `
