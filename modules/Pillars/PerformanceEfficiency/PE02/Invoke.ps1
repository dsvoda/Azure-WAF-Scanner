...
Register-WafCheck -Pillar 'Performance Efficiency' -Id 'PE:02' -Name 'Capacity planning' -Description 'Advisor performance recommendations' -InvokeScript {
  param([string]$SubscriptionId)

  $adv = Get-Advisor -SubscriptionId $SubscriptionId -Category @('Performance')
  $status = ($adv.Count -gt 0) ? 'Warn' : 'Pass'
  New-WafResult -Pillar 'Performance Efficiency' -Id 'PE:02' -Name 'Capacity planning' -Description 'Advisor performance recommendations' `
