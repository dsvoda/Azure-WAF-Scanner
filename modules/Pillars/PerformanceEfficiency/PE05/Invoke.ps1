...
Register-WafCheck -Pillar 'Performance Efficiency' -Id 'PE:05' -Name 'Optimize scaling & partitioning' -Description 'Autoscale settings presence' -InvokeScript {
  param([string]$SubscriptionId)

  $auto = Get-AzAutoScaleSetting -ErrorAction SilentlyContinue
  $status = ($auto.Count -gt 0) ? 'Pass' : 'Warn'
  New-WafResult -Pillar 'Performance Efficiency' -Id 'PE:05' -Name 'Optimize scaling & partitioning' -Description 'Autoscale settings present' `
