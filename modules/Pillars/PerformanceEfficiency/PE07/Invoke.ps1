...
Register-WafCheck -Pillar 'Performance Efficiency' -Id 'PE07' -Name 'Optimize code & infrastructure' -Description 'Advisor performance items' -InvokeScript {
  param([string]$SubscriptionId)

  $adv = Get-Advisor -SubscriptionId $SubscriptionId -Category @('Performance')
  $status = ($adv.Count -gt 0) ? 'Warn' : 'Pass'
  New-WafResult -Pillar 'Performance Efficiency' -Id 'PE07' -Name 'Optimize code & infrastructure' -Description 'Advisor performance items' `
