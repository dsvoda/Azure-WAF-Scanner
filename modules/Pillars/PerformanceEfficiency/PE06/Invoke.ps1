...
Register-WafCheck -Pillar 'Performance Efficiency' -Id 'PE:06' -Name 'Performance testing' -Description 'Automated load tests in staging' -InvokeScript {
  param([string]$SubscriptionId)
  try { $lt = Get-AzLoadTest -ErrorAction Stop; $status = ($lt.Count -gt 0) ? 'Pass' : 'Manual' }
  catch { $status = 'Manual'; $lt = @() }
  New-WafResult -Pillar 'Performance Efficiency' -Id 'PE:06' -Name 'Performance testing' `
