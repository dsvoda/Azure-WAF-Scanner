...
Register-WafCheck -Pillar 'Performance Efficiency' -Id 'PE:12' -Name 'Continuously optimize performance' -Description 'Advisor items trending (approx)' -InvokeScript {
  param([string]$SubscriptionId)

  $adv = Get-Advisor -SubscriptionId $SubscriptionId -Category @('Performance')
  $status = ($adv.Count -gt 0) ? 'Warn' : 'Pass'
  New-WafResult -Pillar 'Performance Efficiency' -Id 'PE:12' -Name 'Continuously optimize performance' -Description 'Open Advisor perf items' `
