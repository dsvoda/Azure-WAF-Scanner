...
Register-WafCheck -Pillar 'Reliability' -Id 'RE:06' -Name 'Scaling strategy' -Description 'Autoscale settings for AppSvc/VMSS/AKS' -InvokeScript {
  param([string]$SubscriptionId)

  $auto = Get-AzAutoScaleSetting -ErrorAction SilentlyContinue
  $status = ($auto.Count -gt 0) ? 'Pass' : 'Warn'
  New-WafResult -Pillar 'Reliability' -Id 'RE:06' -Name 'Scaling strategy' -Description 'Autoscale settings presence' `
