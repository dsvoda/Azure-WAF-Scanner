...
Register-WafCheck -Pillar 'Reliability' -Id 'RE:04' -Name 'Reliability targets & health model' -Description 'Alert rules tied to SLOs; presence check' -InvokeScript {
  param([string]$SubscriptionId)

  $alerts = Get-AzMetricAlertRuleV2 -ErrorAction SilentlyContinue
  $status = ($alerts.Count -gt 0) ? 'Warn' : 'Manual'
  New-WafResult -Pillar 'Reliability' -Id 'RE:04' -Name 'Reliability targets & health model' -Description 'Metric alert rules present (proxy for SLOs)' `
