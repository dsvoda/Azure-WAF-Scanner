...
Register-WafCheck -Pillar 'Operational Excellence' -Id 'OE:12' -Name 'Deployment failure mitigation' -Description 'Feature flags/App Configuration presence' -InvokeScript {
  param([string]$SubscriptionId)

  try { $appcfg = Get-AzAppConfigurationStore -ErrorAction Stop; $status = ($appcfg.Count -gt 0) ? 'Warn' : 'Manual' } catch { $status = 'Manual' }
  New-WafResult -Pillar 'Operational Excellence' -Id 'OE:12' -Name 'Deployment failure mitigation' -Description 'Feature flags/rollback patterns recommended' `
