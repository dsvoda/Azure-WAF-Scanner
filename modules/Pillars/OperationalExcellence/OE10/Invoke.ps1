...
Register-WafCheck -Pillar 'Operational Excellence' -Id 'OE10' -Name 'Automation for lifecycle & governance' -Description 'Policy assignments as governance automation' -InvokeScript {
  param([string]$SubscriptionId)

  $assign = Get-AzPolicyAssignment -ErrorAction SilentlyContinue
  $status = ($assign.Count -gt 0) ? 'Pass' : 'Warn'
  New-WafResult -Pillar 'Operational Excellence' -Id 'OE10' -Name 'Automation for lifecycle & governance' -Description 'Policy assignments present' `
