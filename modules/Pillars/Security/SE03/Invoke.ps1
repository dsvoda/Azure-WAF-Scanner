...
Register-WafCheck -Pillar 'Security' -Id 'SE:03' -Name 'Data classification & labeling' -Description 'Purview/AIP presence (approx)' -InvokeScript {
  param([string]$SubscriptionId)

  try { $pv = Get-AzPurviewAccount -ErrorAction Stop; $status = ($pv.Count -gt 0) ? 'Warn' : 'Manual' } catch { $status = 'Manual' }
  New-WafResult -Pillar 'Security' -Id 'SE:03' -Name 'Data classification & labeling' -Description 'Purview account presence' `
